...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2, zero_init_residual=False,
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(True),  # 定义激活函数
            nn.Dropout(p=0.5),
            nn.Linear(512 * block.expansion, 512 * block.expansion),  # 定义第二个全连接层
            nn.ReLU(True),
            nn.Linear(512 * block.expansion, num_classes)  # 最后一个全连接层。num_classes：分类的类别个数。
        )
...
def resnet26(pretrained=False, progress=True, **kwargs):
    return _resnet('resnet26', Bottleneck, [1, 2, 4, 1], pretrained, progress,
                   **kwargs)
