...
# 先提取fold1的train和val,用来判断使用什么svm参数
...
for C in C_values:
    for kernel in kernel_values:
        svm_classifier = SVC(C=C, kernel=kernel, probability=True)
        svm_classifier.fit(fold1_train_features, fold1_train_labels)
        y_pred_valid = svm_classifier.predict_proba(fold1_val_features)[:, 1]
        y_pred_train = svm_classifier.predict_proba(fold1_train_features)[:, 1]
        auc_valid = roc_auc_score(fold1_val_labels, y_pred_valid)
        auc_train = roc_auc_score(fold1_train_labels, y_pred_train)
        print(f"C={C}, kernel={kernel}, AUC={auc_valid:.3f}, train_auc = {auc_train:.3f}")

        if auc_valid > best_auc:
            best_auc = auc_valid
            best_params = {'C': C, 'kernel': kernel}
            joblib.dump(svm_classifier, './pkl/svm_node_model.pkl')
print(best_params)
...
#     support_vector_machine = SVC(C=0.01, kernel='linear', probability=True)
#     support_vector_machine.fit(train_features, train_labels)
#     y_pred_train = support_vector_machine.predict_proba(train_features)[:, 1]
#
...
#     y_pred_valid = support_vector_machine.predict_proba(val_features)[:, 1]
#
...
#     y_pred_test0 = support_vector_machine.predict_proba(test0_features)[:, 1]
#
...
#     y_pred_test1 = support_vector_machine.predict_proba(test1_features)[:, 1]
#
...
#     y_pred_test2 = support_vector_machine.predict_proba(test2_features)[:, 1]
#     # train
...
#     val_fold_acc, val_fold_sens, val_fold_spec, val_fold_ppv, val_fold_npv = cal_acc_sens_spec_ppv_npv(train_best_threshold, val_labels,
#                                                                                    y_pred_valid)
...
#     test0_fold_acc, test0_fold_sens, test0_fold_spec, test0_fold_ppv, test0_fold_npv = cal_acc_sens_spec_ppv_npv(train_best_threshold, test0_labels,
#                                                                                    y_pred_test0)
...
#     test1_fold_acc, test1_fold_sens, test1_fold_spec, test1_fold_ppv, test1_fold_npv = cal_acc_sens_spec_ppv_npv(train_best_threshold, test1_labels,
#                                                                                    y_pred_test1)
...
#     test2_fold_acc, test2_fold_sens, test2_fold_spec, test2_fold_ppv, test2_fold_npv = cal_acc_sens_spec_ppv_npv(train_best_threshold, test2_labels,
#                                                                                    y_pred_test2)
