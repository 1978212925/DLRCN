...
for i in range(len(all_csv_name)):
    ...
    for fold in range(1, 6):
        ...
        DLRN = np.array(data.iloc[:, 7]).astype(float)
    ...
    print(
        f'DLRN模型预测良性错误{benign_wrong_count_mean}个,95置信区间是{benign_wrong_count_CI}, 良性个数是{benign_count_mean}')
    print(
        f'DLRN模型预测恶性错误{malignant_wrong_count_mean}个,95置信区间是{malignant_wrong_count_CI}, 恶性个数是{malignant_count_mean}')
